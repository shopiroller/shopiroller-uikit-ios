    ...
    func getQueryArray() -> [URLQueryItem] {
        ...
        if(!categoryIds.selectedIds.isEmpty) {
            urlQueryItems.append(
                contentsOf:
                    categoryIds.selectedIds.map{
                        URLQueryItem(
                            name: SRAppConstants.Query.Keys.categoryId,
                            value: $0)
                    })
        }
        if(!brandIds.selectedIds.isEmpty) {
            urlQueryItems.append(contentsOf: brandIds.selectedIds.map{
                URLQueryItem(
                    name: SRAppConstants.Query.Keys.brandId,
                    value: $0) })
        }
        if(!variationGroups.isEmpty) {
            ...
            urlQueryItems.append(contentsOf: selectionIds.selectedIds.map{
                URLQueryItem(
                    name: SRAppConstants.Query.Keys.variantData,
                    value: $0 ) })
        }
        if let minimumPrice = minimumPrice {
            urlQueryItems.append(
                URLQueryItem(
                    name: SRAppConstants.Query.Keys.priceMin,
                    value: String(minimumPrice)))
        }
        if let maximumPrice = maximumPrice {
            urlQueryItems.append(
                URLQueryItem(
                    name: SRAppConstants.Query.Keys.priceMax,
                    value: String(maximumPrice)))
        }
        if(stockSwitch) {
            urlQueryItems.append(
                URLQueryItem(
                    name: SRAppConstants.Query.Keys.stockMin,
                    value: String(1)))
        }
        if(discountedProductsSwitch) {
            urlQueryItems.append(
                URLQueryItem(
                    name: SRAppConstants.Query.Keys.isThereCampaign,
                    value: String(discountedProductsSwitch)))
        }
        if(freeShippingSwitch) {
            urlQueryItems.append(
                URLQueryItem(
                    name: SRAppConstants.Query.Keys.shippingPrice,
                    value: String(true)))
        }
